...
def run_one(image, save_path, w, h, gt_dic=None):
    ...
    pred_image = cv2.copyMakeBorder(image, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=(int(predicted_color_b), int(predicted_color_g), int(predicted_color_r)))
    ...
    if gt_dic is not None:
        ...
        gt_image = cv2.copyMakeBorder(image, 10, 10, 10, 10, cv2.BORDER_CONSTANT, value=(int(gt_color_b), int(gt_color_g), int(gt_color_r)))
        # concat
...
# dataset
testingset = ImageRegressionDataset(root_dir='/root/autodl-tmp/dora_font/', annotation_file='/root/autodl-tmp/dora_font/train/train.txt',
                                        transform=transforms.ToTensor())

# Prepare the image for inference
for i in range(len(testingset)):
    image, w, h, fontsize, fontweight, color_r, color_g, color_b = testingset[i]

    run_one(image, f'./out/sim{i}.jpg', w, h, gt_dic={'fontsize': fontsize, 'fontweight': fontweight, 'color_r': color_r, 'color_g': color_g, 'color_b': color_b})